...
$$
F_{assess,user} = \frac{F_{assess,agent,pre} \cdot F_{assess,user,pre}}
{\sum_{users} F_{assess,user,pre}}
$$
...
$$R_{response,operator} = \sum_{d} \sum_{h \in H_d} (F_{response,agent,h} - \sum_{i} F_{response,user,h}) - F_{assess,agent}$$
